...
@pytest.mark.skip(reason="Requires updated utils.py with GraphData and read_graphs_from_g6")
def test_read_graphs_from_g6():
    from utils import read_graphs_from_g6
    graphs = read_graphs_from_g6("dataset/test_graphs.g6")
    assert len(graphs) == 3
    assert graphs[0].num_of_nodes == 5
    assert graphs[0].adjacency_dict == {
...
def test_is_extensible(path_graph_group, mapping, expected):
